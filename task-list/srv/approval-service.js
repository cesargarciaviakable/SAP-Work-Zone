...
module.exports = class ApprovalService extends cds.ApplicationService {
    async init() {
        ...
        this.on('submitForApproval', async (req) => {
            ...
            if (task.status !== 'Done') return req.error(400, 'Task must be Done to submit for approval')

            const existing = await SELECT.one.from(ApprovalRequests)
                .where({ task_ID: taskId, status: 'Pending' })
            
            if (existing) return req.error(409, 'Task already has a pending approval request')
        ...
        this.on('approve', ApprovalRequests, async (req) => {
            ...
            if (request.status === 'Approved') return SELECT.one.from(ApprovalRequests).where({ ID })
            if (request.status === 'Rejected') return req.error(400, 'Request was already rejected')
            if (request.status === 'Pending') return req.error(400, 'Request is not Pending')
        ...
        this.on('reject', ApprovalRequests, async (req) => {
            ...
            if (request.status === 'Rejected') return SELECT.one.from(ApprovalRequests).where({ ID })
            if (request.status === 'Approved') return req.error(400, 'Request was already approved')
            if (request.status === 'Pending') return req.error(400, 'Request is not Pending')
